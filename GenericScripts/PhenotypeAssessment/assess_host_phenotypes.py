...
def _write_events_file(sequences: typing.List[str], run_dir: str) -> None:
    first = (sequences or [""])[0]
    with open(f"{run_dir}/events.cfg", "w") as fp:
        fp.write(f"u begin InjectSequence {first} 0\n")
        fp.write(f"u begin InjectSequence {first} 1\n")
        num_updates_per_trial = 100
...
u {begin_update} InjectSequence {sequence} 0
u {begin_update} KillRectangle 1 0 1 0
u {end_update} DumpHostTaskGrid a=host_task_grid+order={i}+sequence={
...
def assess_host_phenotypes(
    ...
    with tempfile.TemporaryDirectory() as run_dir:
        ...
        host_grid_paths = list(
            glob.glob(
                f"{run_dir}/data/a=host_task_grid+order=*+sequence=*+ext=.dat",
            ),
        )
        assert len(host_grid_paths) == len(sequences)
        sequence_lookup = {
            hashlib.sha256(sequence.encode()).hexdigest(): sequence
    ...
    concat_host_grid_df = pd.concat(host_grid_dfs)
    host_phen_df = (
        concat_host_grid_df[concat_host_grid_df["Site"] == 1]
        .drop(
            labels=["Site", "Row", "Col"],
            axis="columns",
        )
    ...
    host_phen_df["Viable"] = host_phen_df["Alive"]

    for i, task in enumerate(iter_environment_tasks(environment_content)):
        host_phen_df[f"Task {task}"] = host_phen_df[f"Trait {i}"]

    host_phen_df["Phenotype"] = [
        "inviable"
        if not row["Viable"]
        else "".join(
            str(int(value)) for col, value in row.items() if col.startswith("Trait ")
        )
        for __, row in host_phen_df.iterrows()
    ]
    host_phen_df["Phenotype Traits"] = [
        "".join(
            str(int(value)) for col, value in row.items() if col.startswith("Trait ")
        )
        for __, row in host_phen_df.iterrows()
    ]
    host_phen_df["Num Traits"] = [
        sum(
            int(value)
