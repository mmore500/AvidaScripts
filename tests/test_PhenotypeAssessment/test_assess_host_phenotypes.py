...
def test_assess_host_phenotypes():
    ...
    phenotype_df = assess_host_phenotypes(
        [sequence] * 2,
        environment_content,
        instset_content,
    )
    ...
    assert (
        phenotype_df.drop(columns="Order").iloc[0].to_dict()
        == phenotype_df.drop(columns="Order").iloc[1].to_dict()
    )
